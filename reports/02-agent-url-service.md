...
**Chỉ mục (Indexes):**
- `idx_urls_short_code` (Unique B-Tree): Đẩy nhanh tốc độ tìm kiếm bản ghi theo mã short code.
- `idx_urls_user_id_created` (Composite B-Tree): Hỗ trợ truy vấn danh sách URL của người dùng sắp xếp giảm dần theo thời gian phục vụ cơ chế phân trang (cursor-based pagination).
...
**Chỉ mục (Indexes):**
- `idx_outbox_unpublished` (Partial Index): Chỉ đánh index các bản ghi có `published_at IS NULL` giúp tối ưu hóa tốc độ tìm kiếm các sự kiện chưa gửi.
- `idx_outbox_unpublished_unlocked` (Partial Index): Chỉ đánh index các sự kiện chưa được gửi và chưa bị khóa.
