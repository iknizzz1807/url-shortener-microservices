    ...
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.33\textwidth}
            \begin{claimbox}{accent}
                ...
                \vspace{0.15cm}
                \small Docker Compose, Kubernetes demo, healthcheck, Prometheus, Grafana, smoke test, k6 load test.
            \end{claimbox}
        \end{column}
    \end{columns}
...
\begin{frame}{Redis: nhanh nhưng không phải source of truth}
    ...
    \end{center}
    \begin{claimbox}{accent}
        \small \textbf{Failure semantics:} Redis mất thì redirect fallback về PostgreSQL. Hệ thống chậm hơn nhưng không mất dữ liệu vì PostgreSQL vẫn là source of truth.
    \end{claimbox}
\end{frame}
...
\begin{frame}{Security: đủ lớp cần thiết cho đồ án}
    ...
    \vspace{0.5cm}
    \begin{claimbox}{danger}
        \small \textbf{Production gap:} demo hiện còn dùng secret mẫu, chưa TLS, chưa refresh token/revocation và chưa RBAC/OAuth2 đầy đủ. Các mục này được tách thành hướng hardening thay vì che giấu trong thiết kế.
    \end{claimbox}
\end{frame}
...
\begin{frame}{Scalability: scale theo nút cổ chai, không scale cả cục}
    ...
    \vspace{0.35cm}
    \begin{claimbox}{primary}
        \small \textbf{Horizontal scaling demo:} Docker Compose có overlay scale cho \texttt{url-service=3}; Kubernetes demo triển khai \texttt{url-service} 3 replicas và \texttt{gateway} 2 replicas qua Service discovery.
    \end{claimbox}
\end{frame}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.47\textwidth}
            \begin{claimbox}{accent}
                ...
                \begin{itemize}\small
                    ...
                    \item Kubernetes demo cho kind/minikube.
        ...
        \begin{column}{0.47\textwidth}
            \begin{claimbox}{danger}
                ...
                \begin{itemize}\small
                    \item K8s demo dùng local cluster, chưa production-grade managed cluster.
...
\begin{frame}{Kubernetes Demo \& Production Roadmap}
    \begin{center}
    \begin{tikzpicture}[scale=0.74, transform shape]
    ...
    \vspace{0.3cm}
    \begin{claimbox}{primary}
        \small Repo đã có manifest Kubernetes cho local demo: Namespace, ConfigMap, Secret, Deployments, Services, readiness probes, \texttt{url-service} 3 replicas, \texttt{gateway} 2 replicas và NodePort 30080. Roadmap production còn lại là HPA, Ingress/TLS, managed secrets, tracing và CI/CD.
    \end{claimbox}
\end{frame}
