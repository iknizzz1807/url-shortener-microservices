...
\begin{frame}{Thông điệp chính}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.33\textwidth}
            \begin{claimbox}{primary}
                \centering\large\textbf{Không chỉ là ứng dụng CRUD}\par
                \vspace{0.15cm}
        ...
        \begin{column}{0.33\textwidth}
            \begin{claimbox}{accent}
                \centering\large\textbf{Hỗ trợ triển khai và vận hành}\par
                \vspace{0.15cm}
                \small Docker Compose, Kubernetes deployment, healthcheck, Prometheus, Grafana, smoke test, k6 load test.
            \end{claimbox}
        \end{column}
    \end{columns}
    ...
    \begin{center}
        \begin{claimbox}[width=0.86\textwidth, halign=center]{best}
            \textbf{Luận điểm chính:} hệ thống nhỏ về nghiệp vụ, nhưng bao quát các vấn đề thật của microservices: chia domain, dữ liệu phân tán, messaging, cache, resilience, observability và kiểm thử.
        \end{claimbox}
    \end{center}
\end{frame}
...
\begin{frame}{Giao diện người dùng: luồng rút gọn và theo dõi URL}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.68\textwidth}
            \begin{tcolorbox}[colback=lightgray, colframe=primary, boxrule=1pt, height=0.72\textheight, halign=center, valign=center]
                \vspace{1.2cm}
                \Large\textbf{Frontend Screenshot Placeholder}\par
                \vspace{0.35cm}
                \normalsize Chèn ảnh giao diện app tại đây\par
                \vspace{0.2cm}
                \small Gợi ý file: \texttt{frontend\_screenshot.png}
            \end{tcolorbox}
        \end{column}
...
\begin{frame}{Từ monolith sang microservices: quyết định thiết kế}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.31\textwidth}
            \begin{claimbox}{danger}
                \centering\textbf{Monolith đơn giản}\par\small
    ...
    \vspace{0.35cm}
    \begin{claimbox}{primary}
        \small \textbf{Luận điểm:} URL shortener là bài toán nhỏ nhưng có profile tải rất thực tế: redirect đọc nhiều, analytics ghi nhiều, notification là tác vụ phụ trợ; vì vậy việc tách service dựa trên nhu cầu kỹ thuật thay vì chỉ mang tính hình thức.
    \end{claimbox}
\end{frame}
...
\begin{frame}{Security: các lớp bảo vệ chính}
    ...
    \vspace{0.5cm}
    \begin{claimbox}{danger}
        \small \textbf{Giới hạn triển khai hiện tại:} hệ thống hiện còn dùng secret mẫu, chưa hỗ trợ TLS, refresh token/revocation và RBAC/OAuth2 đầy đủ. Các nội dung này được xác định là hướng mở rộng production hardening.
    \end{claimbox}
\end{frame}
...
\begin{frame}{Observability: giám sát trạng thái vận hành hệ thống}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.50\textwidth}
            \begin{claimbox}{primary}
                \textbf{Signals đã có}
...
\begin{frame}{Load testing: đánh giá khả năng chịu tải và circuit breaker}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.47\textwidth}
            \begin{claimbox}{best}
                \textbf{k6 scenario}
...
\begin{frame}{Mục tiêu benchmark trong tài liệu thiết kế}
    \scriptsize
    \begin{center}
    ...
    \vspace{0.25cm}
    \begin{claimbox}{accent}
        \small Các giá trị trên là mục tiêu benchmark trong tài liệu thiết kế; kết quả thực tế phụ thuộc tài nguyên máy Docker khi chạy thử nghiệm.
    \end{claimbox}
\end{frame}
...
\begin{frame}{Các chi tiết kỹ thuật bổ sung}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.48\textwidth}
            \begin{claimbox}{primary}
                \begin{itemize}\small
...
\begin{frame}{Tóm tắt các quyết định thiết kế chính}
    \scriptsize
    \begin{center}
    \begin{tabular}{p{0.41\textwidth}p{0.49\textwidth}}
        ...
        Data ownership & Không shared database; mỗi service chỉ nhận \texttt{DATABASE\_URL} của chính nó. \\
        ...
        Delivery model & At-least-once messaging; analytics dedup bằng \texttt{processed\_events}. \\
...
\begin{frame}{Phần chi tiết triển khai}
    \begin{center}
        \begin{claimbox}[width=0.82\textwidth, halign=center]{primary}
            \large\textbf{Từ kiến trúc tổng quan đến hiện thực code}\par
            \vspace{0.25cm}
            \small Các slide tiếp theo đi sâu vào service contract, cơ chế sinh short code, RabbitMQ topology, consistency, schema và deployment để chứng minh từng quyết định kiến trúc đã được hiện thực trong source code.
        \end{claimbox}
    \end{center}
\end{frame}
...
\begin{frame}{Short Code: thiết kế mã ngắn và xử lý collision}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.49\textwidth}
            \begin{claimbox}{primary}
        ...
        \begin{column}{0.47\textwidth}
            \begin{claimbox}{accent}
                ...
                \begin{itemize}\small
                    ...
                    \item Với 1 triệu URL, xác suất collision rất thấp theo tính toán trong tài liệu thiết kế.
        ...
        \begin{column}{0.50\textwidth}
            \begin{claimbox}{primary}
                ...
                \begin{itemize}\small
                    ...
                    \item Consumer không gọi chéo sang service khác khi event đã đủ payload.
                \end{itemize}
            \end{claimbox}
        \end{column}
    \end{columns}
        ...
        \begin{column}{0.48\textwidth}
            \begin{claimbox}{best}
                ...
                \begin{itemize}\small
                    ...
                    \item Milestone unique constraint tránh phát sinh nhiều thông báo trùng lặp.
                \end{itemize}
            \end{claimbox}
        \end{column}
    \end{columns}
...
\begin{frame}{Schema Design: chi tiết data ownership}
    \scriptsize
    \begin{center}
    \begin{tabular}{>{\bfseries}p{0.18\textwidth}p{0.34\textwidth}p{0.38\textwidth}}
        ...
        \texttt{url\_db} & \texttt{urls}, \texttt{outbox} & URL là nguồn dữ liệu chính; outbox là cầu nối sang messaging. \\
    ...
    \vspace{0.3cm}
    \begin{claimbox}{secondary}
        \small \textbf{Các chỉ mục chính:} \texttt{short\_code} cho redirect, \texttt{user\_id + created\_at} cho list URL, \texttt{outbox unpublished} cho worker, \texttt{processed\_events(event\_id)} cho dedup.
    \end{claimbox}
\end{frame}
...
\begin{frame}{Grafana Dashboard: quan sát circuit breaker khi kiểm thử tải}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.68\textwidth}
            \begin{tcolorbox}[colback=lightgray, colframe=accent, boxrule=1pt, height=0.72\textheight, halign=center, valign=center]
                \vspace{1.2cm}
                \Large\textbf{Grafana Dashboard Placeholder}\par
                \vspace{0.35cm}
                \normalsize Chèn ảnh dashboard Grafana tại đây\par
                \vspace{0.2cm}
                \small Gợi ý file: \texttt{grafana\_dashboard.png}
            \end{tcolorbox}
        \end{column}
        \begin{column}{0.28\textwidth}
            \begin{claimbox}{accent}
